...
def inference(name, model, device, data_loader, criterion, limit=None):
    ...
    return auc, np.mean(avg_loss), task_aucs, task_outputs, task_targets
...
def main(cfg):
    ...
    test_auc, test_loss, task_aucs, task_outputs, task_targets = inference(name='Inference',
                                               model=model,
                                               device=device,
...
def get_args_parser():
    ...
    parser.add_argument("--dataset_dir", type=str, default="./data/", required=False, help="Datasets directory")
    parser.add_argument("--test_data", type=str, default="cx", required=False, help="Test dataset")
    parser.add_argument("--device", type=str, default="cuda", help="Compute architecture to use. One of ['cpu', 'cuda']")
    ...
    parser.add_argument("--num_batches", type=int, default=29420, help="Number of mini-batches to use")
...
if __name__ == "__main__":
    ...
    task_outputs, task_targets = main(configuration)
    pd.DataFrame(task_outputs).to_csv('/home/linuxu/Desktop/project/results/cx/task_outputs.csv', index=False)
    pd.DataFrame(task_targets).to_csv('/home/linuxu/Desktop/project/results/cx/task_targets.csv', index=False)


min_length = min(len(arr) for arr in task_outputs.values())
task_outputs_trimmed = {key: arr[:min_length] for key, arr in task_outputs.items()}
pd.DataFrame(task_outputs_trimmed).to_csv('/home/linuxu/Desktop/project/results/cx/task_outputs.csv', index=False)
min_length = min(len(arr) for arr in task_targets.values())
task_targets_trimmed = {key: arr[:min_length] for key, arr in task_targets.items()}
pd.DataFrame(task_targets_trimmed).to_csv('/home/linuxu/Desktop/project/results/cx/task_targets.csv', index=False)
